...
The product is split into session status UI, Network Broker, and GUI Executor processes.
...
The session UI, Network Broker, and GUI Executor set both core-dump resource limits to zero before
...
After local session selection, the app installs the exact versioned session authorization and leaves the user's foreground application and all
...
Session UI or Executor XPC invalidation immediately cancels the active relay.
...
A trusted remote turn stop pauses only the current turn, not the authorized device
...
session UI cleanup, while the machine lock remains held across the paused turn interval.
All safety-critical Broker calls to the Executor and session UI have a local
...
An invalid relay frame or Executor loss asks the still-connected session UI to
...
same session authorization. Unknown-status actions are never replayed across
that boundary.
...
only the exact prior authorization mode, policy version, and scopes
...
Installing a session authorization and activating a relay each require a fresh, short-lived Ed25519 proof from the
...
resolved application digest, selected window ID, display fingerprint, and state
generation. The Executor retains at most one current mapping per resolved
application. Switching between eligible applications preserves each application's
...
application, process, window, display, lease, sequence, and session-authorization
...
`set_value` and selection require active full-trust authorization. Secure/password fields and
...
display, lease, sequence, and authorization state, but do not create an image. Pixel
